...
export const Route = createFileRoute("/_authenticated/new-job")({
  head: () => ({
    meta: [
      { title: "Create New Job | Marvellous Jewellers CRM" },
...
type ItemForm = { itemType: string; service: string; metal: string; stone: string; description: string };
const emptyItem: ItemForm = { itemType: "", service: "", metal: "", stone: "", description: "" };

const ITEM_TYPES = ["Ring", "Watch", "Necklace", "Bracelet", "Chain", "Pendant", "Earrings"];
const SERVICES = ["Resize", "Polish", "Stone Replacement", "Battery", "Chain Repair", "Clasp Repair", "Engraving", "Full Service", "Valuation", "Bespoke"];
...
function NewJob() {
  ...
  function validate(): string[] {
    ...
    if (form.fullName.trim().length < 2) problems.push("Customer name is required.");
    if (form.phone.trim().length < 6) problems.push("Customer phone number is required.");
    if (!form.promisedDate.trim()) problems.push("Promised completion date is required.");
    items.forEach((item, index) => {
      const n = items.length > 1 ? `Item ${index + 1}: ` : "";
      if (!item.itemType.trim()) problems.push(`${n}choose an item type.`);
      if (!item.service.trim()) problems.push(`${n}choose the requested work.`);
      if (!item.metal.trim()) problems.push(`${n}enter the metal.`);
      if (!item.description.trim()) problems.push(`${n}add a description.`);
    });
    return problems;
  }
        ...
        <div className="space-y-5">
              ...
              {items.map((item, index) => (
                <div key={index} className="glass-inset rounded-xl p-4">
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    ...
                    <label className="text-sm"><span className="mb-2 block text-muted-foreground">Metal <span className="text-gold">*</span></span>
                      <input className={field} placeholder="e.g. 18ct White Gold" value={item.metal} onChange={setItem(index, "metal")} />
                    </label>
                    <label className="text-sm"><span className="mb-2 block text-muted-foreground">Stone <span className="text-xs">(optional)</span></span>
                      <input className={field} placeholder="e.g. Diamond" value={item.stone} onChange={setItem(index, "stone")} />
                    </label>
                  </div>
          ...
          <Card step="3" title="Job Details" icon={FileText}>
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <label className="glass-inset flex items-center gap-3 rounded-xl px-4 py-3 text-sm md:col-span-2">
                <input type="checkbox" className="size-4 accent-[var(--gold)]" checked={form.needsQuote}
                  onChange={(e) => setForm((f) => ({ ...f, needsQuote: e.target.checked }))} />
                <span>Quote required first — start this job as <span className="text-gold">To Quote</span> instead of sending it straight to the workshop</span>
              </label>
